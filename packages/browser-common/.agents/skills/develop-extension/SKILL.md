...
client's extension registry. The contract is designed for shared extensions across browser generations. Browser-v1
hosting is currently implemented; browser-v2 adapter and loading support are separate work.
...
- **Do not drop disposables.** Anything returned from `core.onEvent`, `core.registerDynamicEventProperties`,
  `core.onNewSession`, another `Listener`, or a timer wrapper must be stored and disposed in your `dispose()`.
